...
pub struct BitBox {
    communication: communication::HwwCommunication,
...
impl BitBox {
    fn from(
        ...
        Ok(BitBox {
            communication: HwwCommunication::from(device)?,
            noise_config,
        })
    }
    ...
    pub fn unlock_and_pair(self) -> Result<PairingBitBox, Error> {
        self.communication
            .query(&[OP_UNLOCK])
    ...
    fn handshake_query(&self, msg: &[u8]) -> Result<Vec<u8>, Error> {
        let mut framed_msg = vec![OP_HER_COMEZ_TEH_HANDSHAEK];
        framed_msg.extend_from_slice(msg);
        let mut response = self.communication.query(&framed_msg)?;
    ...
    fn pair(self) -> Result<PairingBitBox, Error> {
        ...
        let bb02_handshake_1 = self.handshake_query(&host_handshake_1)?;

        host.read_message_vec(&bb02_handshake_1)
            .or(Err(Error::Noise))?;
        let host_handshake_2 = host.write_message_vec(b"").or(Err(Error::Noise))?;

        let bb02_handshake_2 = self.handshake_query(&host_handshake_2)?;
...
impl PairingBitBox {
    ...
    pub fn wait_confirm(self) -> Result<PairedBitBox, Error> {
...
impl PairedBitBox {
    fn from(communication: communication::HwwCommunication, host: HandshakeState) -> Self {
    ...
    fn query_proto(&self, request: Request) -> Result<Response, Error> {
        ...
        let response = self.communication.query(&encrypted)?;
        if response.is_empty() || response[0] != RESPONSE_SUCCESS {
    ...
    pub fn device_info(&self) -> Result<pb::DeviceInfoResponse, Error> {
        match self.query_proto(Request::DeviceInfo(pb::DeviceInfoRequest {}))? {
    ...
    pub fn root_fingerprint(&self) -> Result<String, Error> {
        match self.query_proto(Request::Fingerprint(pb::RootFingerprintRequest {}))? {
    ...
    pub fn show_mnemonic(&self) -> Result<(), Error> {
        match self.query_proto(Request::ShowMnemonic(pb::ShowMnemonicRequest {}))? {
    ...
    pub fn restore_from_mnemonic(&self) -> Result<(), Error> {
        let now = std::time::SystemTime::now();
        let duration_since_epoch = now.duration_since(std::time::UNIX_EPOCH).unwrap();
        match self.query_proto(Request::RestoreFromMnemonic(
    ...
    pub fn bip85_app_bip39(&self) -> Result<(), Error> {
        self.validate_version(">=9.17.0")?;
        match self.query_proto(Request::Bip85(pb::Bip85Request {
